...
def get_emails(person):
    X = []
    emails = []
    snpts_cache = os.path.join(constant.CACHE_SNPTS_DIR, str(person['id']) + '.json')
    snippets = get_snippets(person, aff=True, usecache=True, cache=snpts_cache, cverbose=True)
    snippets = parser.filt_email(snippets)
    if not snippets:
        return []
    ...
    if len(person['simple_affiliation'].split(' ')) > 11 and eps[0][1] < 0.9:
        ...
        snpts_cache = os.path.join(constant.CACHE_NOAFF_SNPTS_DIR, str(person['id']) + '.json')
